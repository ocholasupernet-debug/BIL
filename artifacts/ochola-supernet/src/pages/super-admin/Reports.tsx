    ...
    <SuperAdminLayout>
      <div style={{ maxWidth: 1100 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16, marginBottom: 28 }}>
          ...
          <Kpi label="Total Customers" value={customers.length} sub={`${activeCustomers} active`} color="#06b6d4" icon={Globe} loading={lc} />
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "20px 24px" }}>
            ...
            {lc ? <div style={{ color: C.muted, textAlign: "center" }}>Loading…</div> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  { label: "Hotspot", count: customers.filter(c => c.type === "hotspot" || !c.type).length, color: "#06b6d4" },
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 14, padding: "20px 24px" }}>
            ...
            {lp ? <div style={{ color: C.muted, textAlign: "center" }}>Loading…</div> : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[
                  { label: "Hotspot", count: plans.filter(p => p.type === "hotspot").length, color: "#06b6d4" },
