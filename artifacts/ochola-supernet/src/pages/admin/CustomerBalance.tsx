        ...
        <div style={{ padding: "1.25rem 1.5rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div style={{ display: "flex", gap: "0.5rem" }}>
            {(["topup", "deduct", "transfer"] as const).map(a => (
              <button key={a} onClick={() => setAction(a)} style={{
                ...
                background: action === a ? "rgba(6,182,212,0.15)" : "rgba(255,255,255,0.04)",
                border: `1px solid ${action === a ? "rgba(6,182,212,0.4)" : "var(--isp-border)"}`,
                color: action === a ? "#06b6d4" : "var(--isp-text-muted)",
              }}>
          ...
          {done ? (
            ...
            <button onClick={handleSubmit} disabled={saving || !amount} style={{
              ...
              background: saving || !amount ? "rgba(6,182,212,0.3)" : "linear-gradient(135deg,#06b6d4,#0284c7)", border: "none", color: "white",
    ...
    <AdminLayout>
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "1.5rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "1rem", marginBottom: "1.5rem" }}>
          {[
            { label: "Total Wallet Balance", value: `KES ${totalBalance.toLocaleString()}`, icon: <Wallet size={18} />, color: "#06b6d4" },
              ...
              {filteredCustomers.map(c => (
                <div key={c.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0.75rem 1rem", borderBottom: "1px solid var(--isp-border)", cursor: "pointer" }}
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(6,182,212,0.1)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: "0.72rem", color: "#06b6d4" }}>
