...
export { validateGeneratedRouterScript } from "../lib/router-script-validation.js";

const router: IRouter = Router();
const TAKEOVER_CONFIRMATION = "TAKE CONTROL";
const TAKEOVER_GRANT_TTL_MS = 10 * 60 * 1000;
const TAKEOVER_PLAN = [
  "A verified RouterOS binary backup and text export are created before service changes.",
...
function signInstallerGrant(payload: string): string {
  return createHmac("sha256", installerGrantSecret()).update(payload).digest("base64url");
}
...
function createTakeoverGrant(adminId: number, routerId: number): string {
  ...
  return `tko.v1.${payload}.${signInstallerGrant(payload)}`;
}
...
function createInstallerGrant(adminId: number, routerId: number): string {
  ...
  const nonce = randomBytes(16).toString("hex");
  const payload = `${adminId}.${routerId}.${expiresAt}.${nonce}`;
  return `inst.v1.${payload}.${signInstallerGrant(payload)}`;
}

function verifyInstallerGrant(grant: string, routerId: number): { adminId: number } | null {
  if (!installerGrantSecret()) return null;
  ...
  const expected = signInstallerGrant(payload);
...
:set ocholaSyncFullInstallMarker ""`;
  ...
  const existingCoexistenceOpenVpn = managementRouterId
    ? `
:if (!$vpnConfigured) do={
  ...
  const wireGuardAttempt = installationMode === "takeover" && routerWireGuardUrl
    ? `:if ($majorVersion >= 7) do={
${vpnAttempt("wireguard", "wireGuardUrl", "vpn-wireguard.rsc")}
}`
    : "";
  const ipsecAttempt = installationMode === "takeover" && routerIpsecUrl
...
`);
  ...
  const takeoverBackupStem = `ochola-takeover-${Date.now()}`;
  const takeoverBackup = `# TAKEOVER SAFETY BOUNDARY — no service resource is changed before both files exist.
...
# --- Ordered router-management VPN setup ----------------------------------------
# Attempts primary OpenVPN first, then isolated backup OpenVPN. Each child must verify
...
:local vpnFailureSummary ""
  ${routerWireGuardUrl
    ? versionedUrlAssignment("wireGuardUrl", safeRouterWireGuardUrl, 7)
    : `:set wireGuardUrl ""`}
  ${routerIpsecUrl
    ? versionedUrlAssignment("ipsecUrl", safeRouterIpsecUrl)
    : `:set ipsecUrl ""`}
...
router.post("/admin/router/self-install/takeover/prepare", requireAdmin(), async (req, res): Promise<void> => {
  ...
  if (!Number.isSafeInteger(routerId) || routerId <= 0 || !adminId) {
    res.status(400).json({ ok: false, error: "A signed-in ISP account and router are required." });
    return;
  }
...
router.get([
  ...
  "/scripts/self-install-mainhotspot/:pathRouterId/:pathAdminId/:pathGrant",
], async (req, res): Promise<void> => {
  ...
  if (!grantIdentity || !adminId || grantIdentity.adminId !== Number(adminId)) {
    res.status(403).type("text/plain").send(
      installationMode === "takeover"
        ...
        : "# Installer authorization is missing, invalid, expired, or scoped to another ISP/router.",
    );
    return;
  }
    ...
    if (rid && (token || adminId)) {
      ...
      if (currentRouter) {
        ...
        if (installationMode === "takeover") {
          ...
          const installerGrantQuery = `grant=${encodeURIComponent(installerGrant)}`;
          coexistenceHotspotUrl = `${origin}/api/scripts/coexistence-hotspot/${encodeURIComponent(rid)}.rsc?${installerGrantQuery}`;
        }
        ...
        if (installationMode === "takeover" && !vpnProvisioningError) {
          try {
            ...
            if (!fallbackProvisioning.ready) throw new Error("VPS fallback VPN reconciliation did not report ready.");
          } catch (error) {
            vpnProvisioningError = error instanceof Error ? error.message : String(error);
          }
        }
...
router.get([
  ...
  try {
    ...
    if (protocol === "openvpn" || protocol === "openvpn-backup") {
      ...
      if ((!material && !routerWireGuardFallbackConfigured(routerId)) ||
          !validVpnEndpoint(endpoint) ||
      ...
      const endpointPort = material?.endpointPort ??
        (Number.parseInt(String(process.env.ROUTER_WIREGUARD_PORT ?? "51820"), 10) || 51820);
      ...
      if ((!material && !routerIpsecFallbackConfigured(routerId)) ||
          !validVpnEndpoint(endpoint) || preSharedKey.length < 8) {
        sendRouterVpnError(503,
          "# OCHOLA_ROUTER_VPN_ERROR\n" +
...
router.get("/scripts/coexistence-hotspot/:routerId.rsc", async (req, res): Promise<void> => {
  ...
  const authorization = installationMode === "takeover"
  ...
  if (!authorization) {
    sendCoexistenceError(
      403,
      installationMode === "takeover"
        ? "# Takeover authorization is invalid, expired, or scoped to another router."
        : "# Router installer authorization is invalid, expired, or scoped to another router.",
    );
    return;
  }
...
router.get("/scripts/router-vpn-manual/:routerId/:adminId/:rosVersion/:grant", async (req, res): Promise<void> => {
  ...
  const authorization = installationMode === "takeover"
    ...
    : verifyInstallerGrant(grant, routerId);
  if (!authorization || authorization.adminId !== adminId) {
    sendManualVpnError(403, `${installationMode === "takeover" ? "Takeover" : "Manual VPN"} authorization is invalid, expired, or scoped to another router.`);
    return;
  }
...
router.get("/scripts/:name", async (req, res): Promise<void> => {
  ...
  try {
    ...
    const lines: string[] = [
      ...
      `# login.html carries the captive-portal M-Pesa STK prompt.`,
      `# Tenant-specific prompt settings are embedded when Hotspot Settings deploys`,
      `# the branded login.html and rlogin.html files directly to this router.`,
      ...
      `# Hotspot walled garden — allow the tenant portal before login`,
      safeRos(`/ip hotspot walled-garden ip remove [find where server=hotspot1 and dst-host="${portalHost}"]`, "walled garden tenant cleanup"),
      safeRos(`/ip hotspot walled-garden ip add server=hotspot1 dst-host="${portalHost}" action=accept`, "walled garden tenant portal"),
      ...
      `# Enable RouterOS API services for the dedicated management tunnels.`,
      `:do { /ip service set [find name="api"] disabled=no address=10.8.5.0/24,10.8.6.0/24 } on-error={ :put "  WARN: could not enable API service" }`,
      `:do { /ip service set [find name="api-ssl"] disabled=no address=10.8.5.0/24,10.8.6.0/24 } on-error={ :put "  WARN: could not enable API-SSL service" }`,
      safeRm(`/ip firewall filter remove [find comment="${safeCompanyName} - allow API"]`),
      safeRm(`/ip firewall filter remove [find comment="${safeCompanyName} - allow legacy API"]`),
      ...
      `:do { /ip firewall filter add chain=input protocol=tcp dst-port=8728,8729 src-address=10.8.5.0/24 action=accept comment="DO NOT DELETE - ${safeCompanyName} management API" place-before=0 } on-error={ :do { /ip firewall filter add chain=input protocol=tcp dst-port=8728,8729 src-address=10.8.5.0/24 action=accept comment="DO NOT DELETE - ${safeCompanyName} management API" } on-error={ :put "  WARN: API allow (primary VPN) rule failed" } }`,
      `:do { /ip firewall filter add chain=input protocol=tcp dst-port=8728,8729 src-address=10.8.6.0/24 action=accept comment="DO NOT DELETE - ${safeCompanyName} backup management API" place-before=0 } on-error={ :do { /ip firewall filter add chain=input protocol=tcp dst-port=8728,8729 src-address=10.8.6.0/24 action=accept comment="DO NOT DELETE - ${safeCompanyName} backup management API" } on-error={ :put "  WARN: API allow (backup VPN) rule failed" } }`,
      `:do { /ip firewall filter add chain=input protocol=tcp dst-port=8728,8729 src-address=10.8.0.0/24 action=accept comment="DO NOT DELETE - ${safeCompanyName} legacy API" place-before=0 } on-error={ :do { /ip firewall filter add chain=input protocol=tcp dst-port=8728,8729 src-address=10.8.0.0/24 action=accept comment="DO NOT DELETE - ${safeCompanyName} legacy API" } on-error={ :put "  WARN: API allow (legacy VPN) rule failed" } }`,
      `:do { /ip firewall filter add chain=input protocol=tcp dst-port=8728,8729 src-address=${bridgeIp}/24 action=accept comment="DO NOT DELETE - ${safeCompanyName} LAN management API" place-before=0 } on-error={ :do { /ip firewall filter add chain=input protocol=tcp dst-port=8728,8729 src-address=${bridgeIp}/24 action=accept comment="DO NOT DELETE - ${safeCompanyName} LAN management API" } on-error={ :put "  WARN: API allow (LAN) rule failed" } }`,
      ...
      safeRm(`/interface ovpn-client remove [find name=ocholasupernet comment="mainbillingvpn"]`),
      ...
      `# === OVPN Management Tunnel (cert-based auth) ===`,
        ovpnAdd(routerSlug, `name=${managementClientInterfaceName} connect-to="${routerVpnHost}" port=${routerManagementVpnPortForRouter(router_row.id)} mode=ip cipher=aes128 auth=sha1 add-default-route=no disabled=no comment="DO NOT DELETE - OcholaSupernet management VPN"`, openVpnCredentials.password),
      ``,
      ...
      `# Idempotent: existing users are refreshed without deleting management identities.`,
      `# Preserve the existing router API account and keep both management identities available.`,
      `:local ocholaManagementComment "DO NOT DELETE - OcholaSupernet management API"`,
      `:local ocholaManagementUser [/user find where name="ocholasupernet"]`,
      `:if ([:len $ocholaManagementUser] = 0) do={`,
      `  :do { /user add name="ocholasupernet" password="${routerApiPassword}" group=full comment=$ocholaManagementComment } on-error={ :put "  WARN: ocholasupernet user could not be created" }`,
      `} else={`,
      `  :do { /user set $ocholaManagementUser password="${routerApiPassword}" group=full disabled=no comment=$ocholaManagementComment } on-error={ :put "  WARN: ocholasupernet user could not be updated" }`,
      `}`,
      `:local routerManagementUser [/user find where name="${routerSlug}"]`,
      `:if ([:len $routerManagementUser] = 0) do={`,
      `  :do { /user add name="${routerSlug}" password="${routerApiPassword}" group=full comment="DO NOT DELETE - ${companyName} router management API" } on-error={ :put "  WARN: router management user could not be created" }`,
      `} else={`,
      `  :do { /user set $routerManagementUser password="${routerApiPassword}" group=full disabled=no comment="DO NOT DELETE - ${companyName} router management API" } on-error={ :put "  WARN: router management user could not be updated" }`,
      `}`,
      `:put "      API users ready: ocholasupernet + ${routerSlug} (DO NOT DELETE)"`,
