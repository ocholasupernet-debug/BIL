    ...
    try {
      ...
      if (port.id === 7) {
        ...
        const [
          ...
          read(["/ip/hotspot/ip-binding/print", "=.proplist=.id,address,mac-address,type,server,disabled,comment"]),
          read(["/ip/hotspot/walled-garden/ip/print", "=.proplist=.id,dst-host,action,disabled,comment"]),
          read(["/ip/dns/static/print", "=.proplist=.id,name,address,type,disabled,comment"]),
          read(["/ip/dns/print", "=.proplist=allow-remote-requests,servers,cache-size,max-udp-packet-size"]),
          read(["/ip/firewall/nat/print", "=.proplist=.id,chain,action,src-address,out-interface-list,disabled,comment"]),
          read(["/ip/firewall/filter/print", "=.proplist=.id,chain,action,src-address,dst-address,protocol,src-port,dst-port,in-interface,out-interface,disabled,comment"]),
        ]);
        liveState = {
          bridge,
          bridgePorts,
