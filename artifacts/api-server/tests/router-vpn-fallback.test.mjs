...
test("OpenVPN renders separate RouterOS 6 and 7 compatibility paths", () => {
  ...
  assert.match(ros6, /\/file print file="ochola-router-management-ca-bootstrap"\n\s+:delay 1s\n\s+\/file set \[find name="ochola-router-management-ca-bootstrap\.txt"\] contents=\$ocholaExpectedCa/);
  assert.match(ros6, /\/file remove \[find name="ochola-router-management-ca-https-root\.txt"\]/);
  assert.match(ros6, /\/file print file="ochola-router-management-ca-https-root"\n\s+:delay 1s\n\s+\/file set \[find name="ochola-router-management-ca-https-root\.txt"\] contents=\$ocholaHttpsCa/);
  ...
  assert.doesNotMatch(ros6, /\/file set \[find name="[^"]+"\] contents=\$ocholaExpectedCa/);
...
test("fallback order is OpenVPN then WireGuard then IPsec and stops after success", () => {
  ...
  assert.match(scriptsRoute, /function rejectMarkedServerResponse/);
  assert.match(scriptsRoute, /OCHOLA_COEXISTENCE_ERROR/);
...
test("VPS setup and runtime status readers use the same management paths", () => {
  ...
  assert.doesNotMatch(setup, /\$\{\$f\}/);
   ...
   assert.match(setup, /CA_FILE="\$PUBLIC_CA_FILE"/);
   assert.match(setup, /PUBLIC_CERT_DIR\/chain\.pem/);
