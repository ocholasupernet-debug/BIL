...
test("installer bootstraps the public CA and validates managed HTTPS fetches", () => {
  ...
  assert.match(scriptsRoute, /\/certificate import file-name="\$caImportFile" name="\$\{ROUTER_HTTPS_CERTIFICATE_NAME\}"\n/);
  assert.doesNotMatch(scriptsRoute, /\/certificate import[^\n]*trusted=yes/);
  assert.match(scriptsRoute, /\/certificate set \[find name="\$\{ROUTER_HTTPS_CERTIFICATE_NAME\}"\] trusted=yes/);
