...
router.get("/scripts/:name", async (req, res): Promise<void> => {
  ...
  try {
    ...
    const lines: string[] = [
      ...
      ``,
      `# === NAT + Firewall ===`,
      `:put "[6/8] Applying firewall, NAT and API access rules..."`,
      ...
      `:do { /ip service set api enabled=yes } on-error={ :put "  WARN: could not enable API service" }`,
      safeRm(`/ip firewall filter remove [find comment="${companyName} - allow API"]`),
      `:do { /ip firewall filter add chain=input protocol=tcp dst-port=8728 src-address=10.8.0.0/24 action=accept comment="${companyName} - allow API" place-before=0 } on-error={}`,
      `:do { /ip firewall filter add chain=input protocol=tcp dst-port=8728 src-address=${bridgeIp}/24 action=accept comment="${companyName} - allow API" place-before=0 } on-error={}`,
      ...
      `:do { /tool fetch url="https://${adminSubdomain}.${baseDomain}/api/vpn/client-cert/${routerSecret}/client.key" dst-path=($storage . "/${routerSlug}.key") mode=https } on-error={ :put "  WARN: client key fetch failed" }`,
