  ...
  for (const current of phaseList(phase)) {
    ...
    if (current === "firewall") {
      lines.push(
        ...
        `} else={`,
        `  /ip firewall filter set $primaryRule chain=input action=accept protocol=tcp dst-port=8728,8729 src-address=10.8.5.0/24 disabled=no comment="${managementComment}"`,
        `  :do { /ip firewall filter move $primaryRule destination=0 } on-error={}`,
        `  :put "Refreshed primary management API rule"`,
        `}`,
        ...
        `  :put "Refreshed backup management API rule"`,
        `}`,
