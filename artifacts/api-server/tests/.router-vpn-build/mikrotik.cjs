...
add chain=srcnat action=masquerade src-address=${lanNetwork} out-interface="${interfaceName}" comment="${tag}-hotspot-to-mgmt-nat"` : "";
  const openVpnOptionalSettings = autoDetectRouterOsMajor ? `# The auto-detected path uses only the portable client properties.
# Avoid version-specific properties that can fail during RouterOS parsing.` : routerOs7 ? `# RouterOS 7 path: certificate verification is mandatory.
:do {
...
:put "${tag}: STEP 4/10 skipped - core firewall and NAT are in networksetup.rsc."` : `# Allow API access from the validated VPN peer.
...
:if ([:len $servicePortIds${index}] > 0) do={
  ...
  const walledGardenSetup = portalHostnames.length > 0 ? portalHostnames.map((hostname) => `:do {
