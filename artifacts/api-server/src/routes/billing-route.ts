...
async function billingConfig(): Promise<BillingConfig> {
  const rows = await sbSelectStrict<BillingConfig>(
...
async function ensureCurrentInvoice(account: AccountRow) {
  ...
  const ledgerRows = await sbSelectStrict<{ amount: number | string }>(
    "revenue_ledger",
  ...
  const [invoice] = await sbUpsertStrict<Record<string, unknown>>(
    "platform_billing_invoices",
    "account_id,billing_period",
    payload,
  );
  return { invoice: invoice ?? payload, config };
}
...
router.get("/billing/current", requireAdmin(), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const result = await ensureCurrentInvoice(account);
    ...
    res.status(503).json({
      ok: false,
      error: error instanceof Error ? error.message : "Could not load platform billing.",
    });
  }
});
...
router.get("/billing/history", requireAdmin(), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const invoices = await sbSelectStrict<Record<string, unknown>>(
      "platform_billing_invoices",
...
router.post("/billing/renew", requireAdmin(), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { invoice } = await ensureCurrentInvoice(account);
    if (!invoice) {
    ...
    res.status(503).json({
      ok: false,
      error: error instanceof Error ? error.message : "Could not prepare the billing payment.",
    });
  }
});
