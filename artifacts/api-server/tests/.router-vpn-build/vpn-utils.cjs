...
var vpn_utils_exports = {};
__export(vpn_utils_exports, {
  describeVpnArchitecture: () => describeVpnArchitecture,
  generateVpsOvpnSetupScript: () => generateVpsOvpnSetupScript
});
module.exports = __toCommonJS(vpn_utils_exports);
...
// src/lib/router-vpn-ip.ts
var ROUTER_VPN_SUBNET = "10.8.5";
var ROUTER_VPN_GATEWAY = `${ROUTER_VPN_SUBNET}.1`;
var ROUTER_VPN_BACKUP_SUBNET = "10.8.6";
var ROUTER_VPN_BACKUP_GATEWAY = `${ROUTER_VPN_BACKUP_SUBNET}.1`;
function routerVpnPeerIp(value) {
...
function generateVpsOvpnSetupScript(opts) {
  const {
    vpsPublicIp,
    vpnPort,
    vpnUsername,
...
PUBLIC_KEY_FILE=""
PUBLIC_CA_FILE=""
for PUBLIC_CERT_DIR in   /etc/letsencrypt/live/isplatty.org-wildcard   /etc/letsencrypt/live/isplatty.org-required-hosts
do
...
if ! $SUDO ip -4 addr show dev "${contract.interfaceName}" 2>/dev/null | grep -Eq "[[:space:]]inet[[:space:]]+${serverGw}([/[:space:]]|$)"; then
...
function describeVpnArchitecture(opts) {
  const {
    vpsPublicIp,
    vpnPort,
    vpnUsername = "admin",
  ...
  return {
    ...
    description: "VPS runs the OpenVPN SERVER. The MikroTik router connects as a CLIENT. The backend API server uses the router's tunnel IP to reach the RouterOS API.",
    vpsServer: {
