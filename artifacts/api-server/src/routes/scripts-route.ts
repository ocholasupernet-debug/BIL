...
router.get("/scripts/:name", async (req, res): Promise<void> => {
  ...
  try {
    ...
    const lines: string[] = [
      ...
      `:foreach x in=[/certificate find where name~"${routerSlug}"] do={ :do { /certificate remove $x } on-error={} }`,
      `:foreach x in=[/certificate find where name~"vpn-ca"]        do={ :do { /certificate remove $x } on-error={} }`,
      ...
      `:do { /certificate set [find where name="${routerSlug}"] trusted=yes } on-error={}`,
      `:delay 3s`,
      `:local certFlags [/certificate get [find where name="${routerSlug}"] flags]`,
