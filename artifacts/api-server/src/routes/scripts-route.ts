...
router.get("/scripts/:name", async (req, res): Promise<void> => {
  ...
  try {
    ...
    const lines: string[] = [
      ...
      ros(`/tool fetch url="${scriptBaseUrl}/../vpn/ca.crt" dst-path=ca.crt mode=https`),
      ros(`/certificate import file-name=ca.crt passphrase=""`),
      `:do { /interface ovpn-client remove [find name=ovpn-mgmt] } on-error={}`,
      ros(`/interface ovpn-client add name=ovpn-mgmt connect-to=${adminSubdomain}.isplatty.org port=1194 mode=ip user="${routerSlug}" auth=sha1 cipher=aes256 add-default-route=no disabled=no comment="${companyName} mgmt"`),
      ``,
