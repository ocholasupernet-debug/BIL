...
interface Backup {
  ...
  failureReason: string | null; artifactName: string | null;
}
...
export default function SuperAdminBackups() {
  ...
  const loadBackups = async () => {
    try {
      const response = await fetch("/api/super-admin/backups", { headers: authHeaders(), cache: "no-store" });
    ...
    <SuperAdminLayout>
      <div style={{ maxWidth: 900 }}>
        ...
        <div style={{ background: "var(--isp-accent-glow)", border: `1px solid var(--isp-accent-glow)`, borderRadius: 12, padding: "14px 20px", marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div><p style={{ fontWeight: 700, color: "white", margin: 0, fontSize: "0.85rem" }}>Automatic Schedule: Not configured</p><p style={{ fontSize: "0.72rem", color: C.sub, margin: "2px 0 0" }}>No supported automatic backup scheduler is configured in this environment. Manual runs below report their real server result.</p></div>
        </div>
